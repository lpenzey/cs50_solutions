// Copies a BMP file

#include <stdio.h>
#include <stdlib.h>

#include "bmp.h"

int main(int argc, char *argv[])
{
    // ensure proper usage
    if (argc != 4)
    {
        fprintf(stderr, "Usage: scale infile outfile\n");
        return 1;
    }

    // remember filenames/scale size
    int scale = atoi(argv[1]);
    char *infile = argv[2];
    char *outfile = argv[3];

    if (scale <= 0 || scale > 100)
    {
        printf("Error: scale must be between 1 and 100\n");
        return 1;
    }



    // open input file
    FILE *inptr = fopen(infile, "r");
    if (inptr == NULL)
    {
        fprintf(stderr, "Could not open %s.\n", infile);
        return 2;
    }

    // open output file
    FILE *outptr = fopen(outfile, "w");
    if (outptr == NULL)
    {
        fclose(inptr);
        fprintf(stderr, "Could not create %s.\n", outfile);
        return 3;
    }

    // read infile's BITMAPFILEHEADER
    BITMAPFILEHEADER bf;
    fread(&bf, sizeof(BITMAPFILEHEADER), 1, inptr);

    // read infile's BITMAPINFOHEADER
    BITMAPINFOHEADER bi;
    fread(&bi, sizeof(BITMAPINFOHEADER), 1, inptr);

    // ensure infile is (likely) a 24-bit uncompressed BMP 4.0
    if (bf.bfType != 0x4d42 || bf.bfOffBits != 54 || bi.biSize != 40 ||
        bi.biBitCount != 24 || bi.biCompression != 0)
    {
        fclose(outptr);
        fclose(inptr);
        fprintf(stderr, "Unsupported file format.\n");
        return 4;
    }

    //instantiate initial width and height
    int initialWidth = bi.biWidth;
    int initialHeight = bi.biHeight;

    //update width and height
    bi.biWidth *= scale;
    bi.biHeight *= scale;

    //determine scanline padding for infile and outfile
    int in_padding = (4 - (initialWidth * sizeof(RGBTRIPLE)) % 4) % 4;
    int out_padding = (4 - (bi.biWidth * sizeof(RGBTRIPLE)) % 4) % 4;

    //find outfile biSizeImage and bfSize
    bi.biSizeImage = ((bi.biWidth * sizeof(RGBTRIPLE) + out_padding) * abs(bi.biHeight));
    bf.bfSize = (bi.biSizeImage + 54);

    // write outfile's BITMAPFILEHEADER
    fwrite(&bf, sizeof(BITMAPFILEHEADER), 1, outptr);

    // write outfile's BITMAPINFOHEADER
    fwrite(&bi, sizeof(BITMAPINFOHEADER), 1, outptr);

    // iterate over infile's scanlines
    for (int i = 0; i < abs(initialHeight); i++)
    {
        for (int j = 0; j < scale; j++)
        {

            //aim pointer at beginning of scanline
            fseek(inptr, (54 + ((initialWidth * 3 + in_padding) * i)), SEEK_SET);

            // iterate over pixels in scanline
            for (int k = 0; k < initialWidth; k++)
            {
                // temporary storage
                RGBTRIPLE triple;

                // read RGB triple from infile
                fread(&triple, sizeof(RGBTRIPLE), 1, inptr);

                //iterate over each pixel n times
                for (int l = 0; l < scale; l++)
                {

                    // write RGB triple to outfile
                    fwrite(&triple, sizeof(RGBTRIPLE), 1, outptr);
                }
            }


            // add padding back to outfile
            for (int m = 0; m < out_padding; m++)
            {
                fputc(0x00, outptr);
            }
        }
    }

    // close infile
    fclose(inptr);

    // close outfile
    fclose(outptr);

    // success
    return 0;
}
